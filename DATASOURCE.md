...
| 項目 | 内容 |
|------|------|
| データ元 | 東京都環境局 `taiki.kankyo.metro.tokyo.lg.jp` |
| 取得方式 | JSON API（`scraper.js` / `server.js`） |
| エンコード | UTF-8 |
| 更新頻度 | 1時間ごとにポーリング |
...
> **JSON APIについての注意事項：**
>
> 本プロジェクトが利用しているJSON APIは、東京都環境局が公式に外部向けに公開したものではありません。
> 新サイト（https://www.taiki.kankyo.metro.tokyo.lg.jp/taikikankyo/realtime/index.html ）の
> フロントエンドJavaScript（`page.realtime.min.js`）をリバースエンジニアリングして特定したものです。
>
> そのため以下のリスクがあります：
> - 予告なくURL構造やデータ形式が変更される可能性がある
> - 外部向けの利用規約やアクセス制限に関する情報が不明
> - レート制限の有無が不明
>
> 利用にあたっては、サイトへの過度な負荷をかけないよう配慮してください。

**JSON APIエンドポイント（新サイト）：**

| URL | 内容 |
|-----|------|
| `data/V501Station.json` | 測定局マスタ |
| `data/V502Item.json` | 測定項目マスタ |
| `data/V504Wd.json` | 風向マスタ（数値コード→方位変換） |
| `data/V505Property.json` | 設定情報（取得可能期間など） |
| `data/hour/{YYYYMM}/{YYYYMMDDHH}.json` | 時間別測定データ |

※ ベースURL: `https://www.taiki.kankyo.metro.tokyo.lg.jp/taikikankyo/`
