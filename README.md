...
```bash
# 1. マイグレーションの適用（新しいマイグレーションファイルがある場合）
npm run db:migrate

# 2. Pages + Functions のデプロイ
npm run deploy

# 3. Cron Worker のデプロイ（src/ 以下を変更した場合）
npm run deploy:cron
```

| コマンド | 用途 | いつ実行するか |
|---|---|---|
| `npm run db:migrate` | D1マイグレーション適用 | `migrations/` に新ファイルを追加した時 |
| `npm run deploy` | Pages + Functions デプロイ | `public/` や `functions/` を変更した時 |
| `npm run deploy:cron` | Cron Worker デプロイ | `src/` を変更した時 |
...
│   └── 0003_add_station_period.sql # 測定局にsd/edカラム追加
├── src/
│   ├── tool.js                # ユーティリティ
